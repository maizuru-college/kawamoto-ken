...
def index(request):
    return render(request, 'kekkatodoke/top1.html')
...
def look(request):
    ...
    return render(request, 'kekkatodoke/lookk.html', {'kekkas': kekkas})
...
def send(request):
    if request.method == 'POST':
        ...
        if request.POST["student_code"] != request.user.username:
            ...
            return render(request, 'kekkatodoke/send.html', {
                'form': form,
                'err': err
            })
        ...
        form = KekkaForm
        err = ""
    return render(request, 'kekkatodoke/send.html', {'form': form, 'err': err})
